...
an optimal coupling matrix that matches elements from two sets. In below, we visualize 
the attained coupling $\hat{\Gamma}$ and its overlay as a bipartite graph.

```{r compute2, echo=FALSE, eval=TRUE, fig.alt="Optimal matching visualization", fig.keep="high", fig.show="hold"}
# initialize and explicitly create an image to ensure figure capture
par(mfrow=c(1,2))
...
On the right figure, the edges connecting the dots represent the optimal coupling between the two empirical measures. The thickness of the edges is proportional to the amount of mass transported between the points, which can be interpreted as the "flow" in the transportation problem.
