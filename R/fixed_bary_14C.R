...
#' @rdname fbary14C
#' @export
fbary14C <- function(support, atoms, marginals=NULL, weights=NULL, lambda=0.1, p=2, ...){
  ## INPUT : EXPLICIT
  name.f    = "fbary14C"
  ...
  if ("init.vec" %in% pnames){
    ...
    if ((length(par_init)!=nsupport)||(any(par_init < 0))){
      stop(paste0("* fbary14C : 'init.vec' should be a vector of length ",nsupport," with nonnegative values."))
    }
  } else {
  ...
  ## COMPUTE
  output = cpp_fixed_sinkhorn14(par_listdxy, par_marginals, par_weights, par_p, par_lbd, par_iter, par_tol, par_show, par_init)
  return(as.vector(output))
}

#' @rdname fbary14C
#' @export
fbary14Cdist <- function(distances, marginals=NULL, weights=NULL, lambda=0.1, p=2, ...){
  ## INPUT : EXPLICIT
  name.f      = "fbary14Cdist"
  ...
  if ("init.vec" %in% pnames){
    ...
    par_init = par_init/base::sum(par_init)
    if ((length(par_init)!=nsupport)||(any(par_init < 0))){
      stop(paste0("* fbary14Cdist : 'init.vec' should be a vector of length ",nsupport," with nonnegative values."))
    }
  } else {
    par_init = rep(1/nsupport, nsupport)
  }
