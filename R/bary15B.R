...
#' @rdname bary15B
#' @export
bary15B <- function(support, atoms, marginals=NULL, weights=NULL, lambda=0.1, p=2, ...){
  ## INPUT : EXPLICIT
  name.f    = "bary15B"
  ...
  if ("init.vec" %in% pnames){
    ...
    if ((length(par_init)!=nsupport)||(any(par_init < 0))){
      stop(paste0("* bary15B : 'init.vec' should be a vector of length ",nsupport," with nonnegative values."))
    }
  } else {
...
#' @rdname bary15B
#' @export
bary15Bdist <- function(distances, marginals=NULL, weights=NULL, lambda=0.1, p=2, ...){
  ## INPUT : EXPLICIT
  name.f      = "bary15Bdist"
  ...
  if ("init.vec" %in% pnames){
    ...
    par_init = par_init/base::sum(par_init)
    if ((length(par_init)!=nsupport)||(any(par_init < 0))){
      stop(paste0("* bary15Bdist : 'init.vec' should be a vector of length ",nsupport," with nonnegative values."))
    }
  } else {
    par_init = rep(1/nsupport, nsupport)
  }
